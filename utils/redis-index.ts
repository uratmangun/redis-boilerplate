 ...
 */
export async function createItemsIndex(config: RedisIndexConfig): Promise<void> {
  ...
  try {
    ...
    */
    ...
    console.log('- Vector fields: titleEmbeddings, contentEmbeddings, combinedEmbeddings');
 ...
 */
export async function checkIndexExists(config: RedisIndexConfig): Promise<boolean> {
  const redis = await connect({
    hostname: new URL(config.redisUrl).hostname,
    port: parseInt(new URL(config.redisUrl).port) || 6379,
    password: new URL(config.redisUrl).password || undefined,
