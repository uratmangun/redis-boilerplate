  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (request.method !== 'POST') {
      return new Response(JSON.stringify({
        success: false,
        message: 'Method not allowed. Use POST to initialize the search index.',
    ...
    try {
      ...
      if (indexExists) {
        return new Response(JSON.stringify({
          success: true,
          message: 'Redis search index already exists and is ready for vector similarity search.',
          indexExists: true,
          timestamp: new Date().toISOString(),
        }), {
          status: 200,
      ...
      const response: InitIndexResponse = {
        success: true,
        message: 'Redis search index created successfully. Vector similarity search is now enabled.',
        indexExists: false,
        timestamp: new Date().toISOString(),
      };
